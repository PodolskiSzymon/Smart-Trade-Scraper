...
response = session.get(f"https://www.vinted.pl/api/v2/items/{item_id}/page")

print(f"Status: {response.status_code}")

# Jeśli status to nie 200 OK, drukujemy to, co API ukryło w odpowiedzi
if response.status_code != 200:
    print("Treść błędu od serwera:")
    try:
        # Próbujemy ładnie wydrukować JSON-a, jeśli serwer go zwrócił
        import json
        #print(json.dumps(response.json(), indent=4))
    except:
        # Jeśli to zwykły tekst, drukujemy surowy tekst
        print(response.text)
else:
    print("sukces")