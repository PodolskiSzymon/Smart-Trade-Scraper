...
from db_management import pobierz_ostatnie_20_id, zapisz_nowe_id;
import time;
import os;
import requests;
import logging;
...
def run_scraper_cycle(session, target_category, target_stop_ids):
    ...
    while curr_page <= last_page:
        ...
        # ... (logika odświeżania ciastka bez zmian) ...
        ...
        for item in items:
            ...
            # ... (TUTAJ LOGIKA POBIERANIA GŁÓWNEGO ZDJĘCIA z request.get) ...
                ...
                if photos:
                    ...
                    if photo_url:
                        ...
                        if image_response.status_code == 200:
                            # Tworzymy folder docelowy (exist_ok=True uodparnia kod na wyrzucenie błędu, jeśli folder już jest)
                            ...
                            print(f"[DYSK] Zapisano zdjęcie główne: {local_path}, z ulr: {item['url']}")
                        else:
                            print(f"[BŁĄD CDN] Serwer odrzucił plik graficzny (Status: {image_response.status_code})")
                else:
                    print(f"[INFO] Brak zdjęć w ogłoszeniu ({item_id}).")
            else:
                print(f"[BŁĄD API] Nie udało się pobrać JSON ze zdjęciami (Status: {photo_response.status_code})")
...
def main():
    ...
    while True:
        ...
        if nowo_pobrane_id:
            zapisz_nowe_id(nowo_pobrane_id)
