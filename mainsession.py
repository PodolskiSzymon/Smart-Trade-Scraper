...
def get_item_details(item: dict) -> dict:
    # 1. Dane finansowe
    price_info = item.get("total_item_price", {})
    service_fee = item.get("service_fee", {})
    
    # 2. Logika dla przedmiotów zagranicznych
    conversion = item.get("conversion")
    is_foreign = conversion is not None
    
    # 3. Logika dla "Podbitych" ogłoszeń
    label = item.get("item_box", {}).get("accessibility_label", "")
    is_boosted = "Podbity" in label
    
    return {
        "id": item.get("id"),
        "title": item.get("title"),
        "price_pln": float(price_info.get("amount", 0)),
        "service_fee_pln": float(service_fee.get("amount", 0)),
        "currency": price_info.get("currency_code"),
        "is_foreign": is_foreign,
        "seller_currency": conversion.get("seller_currency") if is_foreign else "PLN",
        "is_boosted": is_boosted,
        "is_suspicious": item.get("is_suspicious", False),
        "is_1st_discount": item.get("show_1st_time_seller_discount", False),
...
if(response.status_code!=200):
    print(f"blad statusu, zaktualizuj ciasteczka -> vinted->inspekcja->xhr->promoted_clothes->copy bash xpath->wklej do https://curlconverter.com/ \n->skopjuj cookies, wklej do cookies w tym porogramie", response.status_code)
    ...
    for licznik, item in enumerate(items):
            if licznik==1:
                item_data: dict = get_item_details(item)
                print(f"Przedmiot: {item_data['title']}, Podbity: {item_data['is_boosted']}, Zagraniczny: {item_data['is_foreign']}, reszta: {item_data['currency']}")
                # with open("itemy.html", "a", encoding="utf-8") as f:
                #     wstaw=f"Item numer {licznik}\n"
                #     wstaw+= f"Tytul: {item_data['title']}, ID: {item_data['id']}, cena: {item_data['price_pln']}\n opłata vinted: {item_data['service_fee_pln']}, waluta: {item_data['currency']}, czy zagraniczne {item_data['is_foreign']}\n"
                #     wstaw+= f"czy podbite: {item_data['is_boosted']}, czy podejrzane: {item_data['is_suspicious']} czy pierwsza zniżka: {item_data['is_1st_discount']}, marka: {item_data['brand']}\n"
                #     wstaw+= f"link do szczegółów: {item_data['url']}\n polubienia: {item_data['favourite_count']}, stan: {item_data['status']}\n"
                #     wstaw+=f"zdjęcie główne: {item_data['image_url']}\n\n"
                #     f.write(wstaw)
                # ... (poprzedni kod z pętli)
                #item_id = item_data['id']
                item_id=9294313157
                url_tlumaczenia = f"https://www.vinted.pl/api/v2/items/{item_id}/plugins/translatable"
                
                # Parametr z Twojego curla
                params_tlum = {'localize': 'true'}
                
                # Strzelamy w znaleziony przez Ciebie ukryty endpoint
                resp_opis = session.get(url_tlumaczenia, params=params_tlum, headers=headers, cookies=cookies)
                
                opis_przedmiotu = "Brak opisu lub błąd"
                
                if resp_opis.status_code == 200:
                    json_opis = resp_opis.json()
                    # Endpoint tłumaczeń zazwyczaj zwraca oryginalny tekst i tłumaczenie. 
                    # Drukujemy cały JSON do terminala, żebyś na własne oczy zobaczył strukturę dla pierwszego trafienia
                    print("\n--- ZNALEZIONY JSON Z OPISEM ---")
                    print(json_opis)
                    print("--------------------------------\n")
                    with open("opis_konkretnego_ogloszenia.html", "a", encoding="utf-8") as f:
                        dowstawienia=f"opis {licznik} ogloszenia:\n{json_opis}\n\n\n"
                        f.write(dowstawienia)
                    
                    #Zależnie od struktury, którą wypluje powyższy print, 
                    #wyciągnięcie opisu będzie wyglądać mniej więcej tak:
                    opis_przedmiotu = json_opis.get('description', '')
                    time.sleep(delay)
                else:
                    print(f"Odrzucono zapytanie o opis dla ID {item_id}. Status: {resp_opis.status_code}")
                
                # Teraz możesz dodać 'opis_przedmiotu' do zmiennej 'wstaw' i zapisać do pliku!
