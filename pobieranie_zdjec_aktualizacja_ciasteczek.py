...
def get_all_photo_urls(item_id, session, headers, cookies):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[ERROR] Endpoint zdjęć zwrócił status {response.status_code}, aktualizuje cookies, uruchom program ponownie")
...
id="9238023547"  
zdjecia = get_all_photo_urls(id, session, headers, cookies)
os.makedirs("zdjecia", exist_ok=True)
for licznik, z in enumerate(zdjecia):
    print(f"ID: {z['id']} | Główne: {z['is_main']} | URL: {z['url']}")
    # nazwa_pliku=f"zdj{licznik}.png"
    # zdjecie = requests.get(z['url'], stream=True)
    # with open(f"zdjecia/{nazwa_pliku}", 'wb') as f:
    #         for chunk in zdjecie.iter_content(8192):
    #             f.write(chunk)